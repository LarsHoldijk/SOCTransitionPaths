...
class NNPolicy(nn.Module):
    def __init__(self, device, force, dims, T, vel_given=True):
        super().__init__()
        self.device = device
        self.force = force
        self.T = T
    ...
    def forward(self, x_in, t):
        ...
        if not self.force:
            out_grad = -torch.autograd.grad(outputs=out, inputs=x, grad_outputs=torch.ones_like(out), create_graph=True,
                                            retain_graph=True)[0]
